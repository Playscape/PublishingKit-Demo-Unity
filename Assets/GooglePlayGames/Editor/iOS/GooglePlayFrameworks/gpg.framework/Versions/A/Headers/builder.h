...
class GPG_EXPORT GameServices::Builder {
 public:
  ...
  // STOPSHIP(ericrk): Hide this code before shipping. b/18507076
  // Apiary only for now - overriding this on GMS Core is tricky.
  Builder &InternalSetDriveRootURL(std::string const &root_url);
  Builder &InternalSetGamesRootURL(std::string const &root_url);
